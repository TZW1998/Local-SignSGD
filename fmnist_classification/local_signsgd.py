...
parser.add_argument('--lr', type=float, default=0.1, help='Local Learning rate for sgd')
parser.add_argument('--num_nodes', type=int, default=4, help=' Number of computing nodes')
...
parser.add_argument('--global_beta2', type=float, default=0.9, help='Global beta2 for Adam')
...
log_dir = 'logs/fmnist_classification/localsignsgd_exp{}_bz{}_lr{}_glr{}_betas({},{})_wd{}_nn{}_ls{}'.format(args.exp_index, batch_size, args.lr, global_lr, global_betas[0], global_betas[1], global_weight_decay, args.num_nodes, args.local_steps)
if os.path.exists(log_dir):
    clean_dir(log_dir)
logger = SummaryWriter(logdir= log_dir)
print('Start training with Local-SignSgd algorithm, batch size {}, local learning rate {}, global learning rate {}, global betas ({},{}), global weight decay {}, num nodes {}, local steps {}.'.format(batch_size, args.lr, global_lr, global_betas[0], global_betas[1], global_weight_decay, args.num_nodes, args.local_steps))
