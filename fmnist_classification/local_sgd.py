...
log_dir = 'logs/fmnist_classification/localsgd_exp{}_bz{}_lr{}_glr{}_mm{}_wd{}_nn{}_ls{}'.format(args.exp_index, batch_size, args.lr, global_lr, global_momentum, global_weight_decay, args.num_nodes, args.local_steps)
if os.path.exists(log_dir):
    clean_dir(log_dir)
logger = SummaryWriter(logdir= log_dir)
print('Start training with Local-SGD algorithm, batch size {}, local learning rate {}, global learning rate {}, global momentum {}, global weight decay {}, num nodes {}, local steps {}.'.format(batch_size, args.lr, global_lr, global_momentum, global_weight_decay, args.num_nodes, args.local_steps))
...
while gradient_steps < args.total_steps:
    ...
    for n, p in global_weight.items():
        ...
        global_state["momentum"][n].mul_(global_momentum).add_(aggregated_pseudo_gradients[n] / args.num_nodes,alpha= 1 - global_momentum)

        # weight_deacy
        d_p = global_state["momentum"][n] + global_weight_decay * p
